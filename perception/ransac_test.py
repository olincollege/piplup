...
def main():
    ...
    best_eq, best_inliers = cuboid_fit.fit(
        filtered_cloud, thresh=0.02, maxIteration=5000
    )
    ...
    com = [
        np.average(filtered_cloud[:, 0]),
        np.average(filtered_cloud[:, 1]),
        np.average(filtered_cloud[:, 2]),
    ]
    ...
    for eq in best_eq:
        ax.plot(
            [com[0], com[0] + eq[0]], [com[1], com[1] + eq[1]], [com[2], com[2] + eq[2]]
        )
