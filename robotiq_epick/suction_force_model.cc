...
namespace piplup
{
    namespace suction_gripper
    {
        ...
        CupObjInterface::CupObjInterface(
            ...
            const std::pair<drake::geometry::FrameId,
          ...
          , num_suction_cups_(action_point_frames.second.size())
        {
            ...
            DeclarePeriodicUnrestrictedUpdateEvent(
                time_step, 0, &CupObjInterface::UpdateDists);
        }
            ...
            drake::systems::State<double> * state_ptr) const
        {
            ...
            {
                auto action_pt_pose =
                    geom_query.GetPoseInWorld(action_point_frames_.first)
                    * action_point_frames_.second.at(suction_cup_idx);
                drake::geometry::GeometryId closest_obj_geom_id;
                auto min_action_point_dist = std::numeric_limits<double>::infinity();

                auto all_signed_dists =
                    geom_query.ComputeSignedDistanceToPoint(action_pt_pose.translation());
