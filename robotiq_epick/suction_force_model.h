   ...
   public:
    ExternallyAppliedSpatialForcePair(
        ...
        const Eigen::Vector3d&
            force_axis,  // direction of force applied on body1
        double f_mag,
...
class CupPressureSource : public drake::systems::LeafSystem<double> {
   public:
    ...
    /// source (pump or vaccum generator) relative to atmosphere pressure, in
    /// Pa.
...
class CupObjInterface : public drake::systems::LeafSystem<double> {
   public:
    ...
    const drake::systems::OutputPort<double>& GetSuctionForceOutputPort()
        const {
   ...
   private:
    ...
    drake::multibody::BodyIndex gripper_body_;
    const std::pair<drake::geometry::FrameId, std::vector<drake::math::RigidTransformd>> action_point_frames_;
    std::vector<drake::geometry::GeometryIdSet> edge_points_;
